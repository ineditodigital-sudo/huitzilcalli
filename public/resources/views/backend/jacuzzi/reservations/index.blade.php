        ...
        <table class="table aiz-table mb-0 align-middle">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg">{{ __('Fecha') }}</th>
                    <th data-breakpoints="lg">{{ __('Horario') }}</th>
                        ...
                        <div class="row mb-2 px-3 calendar-pill">
                            ...
                            @else
                                ...
                                <div class="col text-center py-1" style="border: 1px solid #272a2540; border-top-right-radius: 0.5rem; border-bottom-right-radius: 0.5rem">
                                    ...
                                    <p class="m-0">{{ date('d/m/Y', strtotime($reservation->start)) }} - Todo el día</p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-sm-6 mb-3 @if(!empty($cabin)) d-none @endif">
                            <label for="basic-url" class="form-label">{{ __('Jacuzzi') }}</label>
                            <div class="input-group has-validation">
                                <select type="text" id="cabin_id" name="cabin_id" class="form-select" placeholder="{{ __('Cabaña') }}" required>
                        ...
                        <div class="col-sm-4 mb-3">
                            <label for="basic-url" class="form-label">{{ __('Fecha') }}</label>
                            <div class="input-group has-validation">
                                <input type="date" id="date" name="date" class="form-control" onchange="checkAvailability()" placeholder="{{ __('Fecha de la reservación') }}" required />
                        ...
                        <div class="col-sm-4 mb-3">
                            <label for="basic-url" class="form-label">{{ __('Horario') }}</label>
                            <div class="input-group has-validation">
                                <select type="text" id="schedule_id" name="schedule_id" class="form-select" placeholder="{{ __('Horario de la reservación') }}" required>
                                    <option value="0" class="fs-5">Todo el día</option>
                                    @foreach($schedules as $schedule)
                                        <option value="{{ $schedule->id }}" class="fs-5">{{ date('H:i', strtotime($schedule->start_time)) }} - {{ date('H:i', strtotime($schedule->end_time)) }} hrs.</option>
                                    @endforeach
                                </select>
...
@section('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function(info) {
                    ...
                    let list = '';
                    $.ajax({
                        url: "{{ route('reservaciones.dia2', ['service_type' => 'jacuzzis']) }}",
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        },
            ...
            if(isMobile()){
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/rev/jacuzzis/mobilejson/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif',
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/res/jacuzzis/json/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif',
                    'https://huitzilcalli.com/admin/dias/json/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif'
                ]);
            }
        ...
        const checkAvailability = (initialValue = null) => {
            ...
            let date = $('#date').val();
            $.ajax({
                url: "{{ route('reservaciones.revisar.jacuzzi') }}",
                data: {
        ...
        const submitData = () => {
            ...
            $.ajax({
                url: "{{ route('reservaciones.jacuzzi.store', ['service_type' => 'jacuzzis']); }}",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
        ...
        const loadFilteredData = (startDate) => {
            ...
            $.ajax({
                url: "{{ route('reservaciones.search') }}?start=" + startDate + "&service_type=jacuzzis", 
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
                    $('#table-body').html(res.table_view);
        ...
        const loadDataByName = (name) => {
            ...
            $.ajax({
                url: "{{ route('reservaciones.searchByName') }}?name=" + name + "&service_type=jacuzzis",   
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
