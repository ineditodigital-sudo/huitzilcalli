            ...
            <section class="col-12 position-fixed top-50 start-50 translate-middle" id="cabins-section">
                <div class="row justify-content-evenly align-items-center">
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6">
                        ...
                        <!--<a href="{{ $services_count==0 ? '#!' : ( $services_count==1 ? ( $service->slug=='cabanas' ? route('detalles', ['slug' => $service_aux->slug]) : route('detallesJacuzzi', ['slug' => $service_aux->slug]) ) : route($service->slug)) }}" class="btn text-decoration-none text-white w-100 d-flex flex-column p-0 border-0 overflow-hidden cabin-card my-2">-->
                        <a href="{{ route($service->slug) }}" class="btn text-decoration-none text-white w-100 d-flex flex-column p-0 border-0 overflow-hidden cabin-card my-2">
                            @if($services_count > 0 && count($service_aux->gallery) > 0)
                                <div class="img-container w-100 d-flex flex-fill" style="border-bottom-left-radius: 2rem; border-bottom-right-radius: 2rem">
                                    <div id="carousel-{{ $service->id }}" class="carousel slide carousel-fade w-100 d-flex flex-fill" data-bs-ride="carousel">
                                        <div class="carousel-inner d-flex flex-fill" style="">
                                            @foreach($service_aux->gallery as $g => $photo)
                            ...
                            <div class="service-title p-3">
                                <div class=" w-100 d-flex justify-content-center align-items-center">
                                    <i class="uil uil-{{ $service->id==1 ? 'estate' : 'swimmer' }} fs-4 me-2 card-icon"></i>
                                    <h1 class="fs-3 mb-0">
                                        {{ $service->name }}
                                    </h1>    
                                </div>
