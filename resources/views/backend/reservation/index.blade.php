                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-sm-6 mb-3 @if(!empty($cabin)) d-none @endif">
                            <label for="basic-url" class="form-label">{{ __('Cabaña') }}</label>
...
@section('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function(info) {
                    ...
                    let list = '';
                    $.ajax({
                        url: "{{ route('reservaciones.dia2', ['service_type' => 'cabañas']) }}",
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        },
            ...
            if(isMobile()){
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/rev/cabañas/mobilejson/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif',
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/res/cabañas/json/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif',
                    'https://huitzilcalli.com/admin/dias/json/@if(!empty($cabin)){{$cabin->id}}@else{{__('0')}}@endif'
                ]);
            }
        ...
        const loadFilteredData = (startDate) => {
            ...
            $.ajax({
                url: "{{ route('reservaciones.search') }}?start=" + startDate + "&service_type=cabañas",
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
                    $('#table-body').html(res.table_view);
        ...
        const loadDataByName = (name) => {
            ...
            $.ajax({
                url: "{{ route('reservaciones.searchByName') }}?name=" + name + "&service_type=cabañas", 
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
