...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        ...
        <div class="col text-end">
            ...
            <div class="btn-group">
                ...
                <button type="button" id="btn-calendar-view" onClick="toggleView('calendar')" class="btn btn-circle btn-outline-info">
                    <span>
    ...
    <form class="form-group" id="reservation-form">
        <div class="row justify-content-start mt-3">
            ...
            <div class="col-md-2">
                <div class="d-flex">
                    <input type="date" name="fechaReserva" id="fechaReserva" class="form-control me-2">
                    
                    <i class="uil uil-multiply" style="align-self: center; display: none;" id="limpiarInput"></i>
                </div>
            </div>
        </div>
    </form>
    ...
    <div class="card-body table-responsive" id="table-section">
        <table class="table aiz-table mb-0 align-middle">
            <thead>
                <tr>
                    <th>#</th>
                    <th><?php echo e(__('Jacuzzi')); ?></th>
                    <th data-breakpoints="lg"><?php echo e(__('Cliente')); ?></th>
                    <th data-breakpoints="lg"><?php echo e(__('Fecha')); ?></th>
                    <th data-breakpoints="lg"><?php echo e(__('Horario')); ?></th>
...
<div id="calendar-section" class="row d-none jacuzzi-calendar">
                ...
                <?php $__currentLoopData = $reservations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r => $reservation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="reservation-card p-2 mb-3 position-relative" style="border-radius: 0.5rem; border: 1px solid #eee">
                        ...
                        <div class="row mb-2 px-3 calendar-pill">
                            <?php if($reservation->full_day_discount != null): ?>
                            ...
                            <?php else: ?>
                                <div class="col-lg-2 col-md-3 col-sm-3 col-2 d-flex justify-content-center align-items-center bg-secondary" style="border-top-left-radius: 0.5rem; border-bottom-left-radius: 0.5rem">
                                    <i class="uil uil-calendar-alt fs-4 text-primary"></i>
                                </div>
                                <div class="col text-center py-1" style="border: 1px solid #272a2540; border-top-right-radius: 0.5rem; border-bottom-right-radius: 0.5rem">
                                    <p class="fw-bold text-uppercase m-0" style="font-size: 0.8rem; color: #dee2e6;">Fecha</p>
                                    <p class="m-0"><?php echo e(date('d/m/Y', strtotime($reservation->start))); ?> - Todo el día</p>
                                </div>
                            <?php endif; ?>
                        </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-sm-6 mb-3 <?php if(!empty($cabin)): ?> d-none <?php endif; ?>">
                            <label for="basic-url" class="form-label"><?php echo e(__('Jacuzzi')); ?></label>
                        ...
                        <div class="col-sm-4 mb-3">
                            <label for="basic-url" class="form-label"><?php echo e(__('Fecha')); ?></label>
                            <div class="input-group has-validation">
                                <input type="date" id="date" name="date" class="form-control" onchange="checkAvailability()" placeholder="<?php echo e(__('Fecha de la reservación')); ?>" required />
                                <span class="invalid-feedback" for="date" role="alert">
                                    <strong></strong>
                                </span>
                            </div>
                        </div>
                        <div class="col-sm-4 mb-3">
                            <label for="basic-url" class="form-label"><?php echo e(__('Horario')); ?></label>
                            <div class="input-group has-validation">
                                <select type="text" id="schedule_id" name="schedule_id" class="form-select" placeholder="<?php echo e(__('Horario de la reservación')); ?>" required>
                                    <option value="0" class="fs-5">Todo el día</option>
                                    <?php $__currentLoopData = $schedules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($schedule->id); ?>" class="fs-5"><?php echo e(date('H:i', strtotime($schedule->start_time))); ?> - <?php echo e(date('H:i', strtotime($schedule->end_time))); ?> hrs.</option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                                <span class="invalid-feedback" for="schedule_id" role="alert">
                                    <strong></strong>
                                </span>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dateClick: function(info) {
                    ...
                    let list = '';
                    $.ajax({
                        url: "<?php echo e(route('reservaciones.dia2', ['service_type' => 'jacuzzis'])); ?>",
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                        },
                        ...
                        success: (res) => {
                            ...
                            if(res.data){
                                res.data.forEach(reservation => {
                                    if(reservation.full_day_discount === null){
                                        list += `<li><strong>${reservation.cabinName}</strong>: ${reservation.customer} (${reservation.startTime} - ${reservation.endTime})</li>`;
                                    }else{
                                        list += `<li><strong>${reservation.cabinName}</strong>: ${reservation.customer} (Todo el día)</li>`;
                                    }
                                });
                            }
            ...
            if(isMobile()){
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/rev/jacuzzis/mobilejson/<?php if(!empty($cabin)): ?><?php echo e($cabin->id); ?><?php else: ?><?php echo e(__('0')); ?><?php endif; ?>',
                ...
                calendar.setOption('eventSources', [
                    'https://huitzilcalli.com/admin/res/jacuzzis/json/<?php if(!empty($cabin)): ?><?php echo e($cabin->id); ?><?php else: ?><?php echo e(__('0')); ?><?php endif; ?>',
                    'https://huitzilcalli.com/admin/dias/json/<?php if(!empty($cabin)): ?><?php echo e($cabin->id); ?><?php else: ?><?php echo e(__('0')); ?><?php endif; ?>'
                ]);
            }
        ...
        const loadData = () => {
            <?php
                $params = [];
                if(!empty($cabin)){
                    $params = ['service_type' => 'jacuzzis', 'id' => 0, 'cabin_id' => $cabin->id];
                }else{
                    $params = ['service_type' => 'jacuzzis', 'id' => 0];
                }
            ?>
            ...
            $.ajax({
                ...
                success: (res) => {
                    ...
                    if(datos){
                        ...
                        $('#date').val(datos.start.split(' ')[0]);
                        $('#notes').val(datos.notes);
                        
                        checkAvailability(datos.schedule_id == null ? 0 : datos.schedule_id);
                        $('#schedule_id').val(datos.schedule_id == null ? 0 : datos.schedule_id);
        ...
        const checkAvailability = (initialValue = null) => {
            let auxId = document.getElementById('reservation_id').value;
            let date = $('#date').val();
            $.ajax({
                url: "<?php echo e(route('reservaciones.revisar.jacuzzi')); ?>",
                data: {
                ...
                success: (data) =>{
                    ...
                    data.forEach(el => {
                        if(el.id == 0){
                            html += `<option value="${el.id}" class="fs-5">Todo el día</option>`;
                        }else{
                            html += `<option value="${el.id}" class="fs-5">${el.startTime} - ${el.endTime} hrs.</option>`;
                        }
                    });
        ...
        const submitData = () => {
            ...
            $.ajax({
                url: "<?php echo e(route('reservaciones.jacuzzi.store', ['service_type' => 'jacuzzis'])); ?>",
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                },
        ...
        const loadFilteredData = (startDate) => {
            <?php
                $params = [
                    ...
                    'service_type' => "jacuzzis"
                ];
            ?>
            
                console.log('Fecha de inicio seleccionada:', startDate); 
            $.ajax({
                url: "<?php echo e(route('reservaciones.search')); ?>?start=" + startDate + "&service_type=jacuzzis", 
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
                    $('#table-body').html(res.table_view);
        ...
        const loadDataByName = (name) => {
            ...
            $.ajax({
                url: "<?php echo e(route('reservaciones.searchByName')); ?>?name=" + name + "&service_type=jacuzzis",   
                method: 'GET',
                success: (res) => {
                    console.log("RES:::::", res);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Maindsoft\Documents\Repos\LaurelesCalvillo\resources\views/backend/jacuzzi/reservations/index.blade.php ENDPATH**/ ?>